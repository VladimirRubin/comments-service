...
def create_dummy_user():
    ...
    while USER_MODEL.objects.filter(username=username).exists():
        username = fake.user_name()
    user = USER_MODEL.objects.create_user(username, email=fake.email(), password=fake.password(8))
    return user
...
def create_dummy_article():
    ...
    article = BlogArticle.objects.create(user=user, title=fake.text())
    return article
...
def create_dummy_page():
    ...
    page = Page.objects.create(user=user, title=fake.text())
    return page
...
def create_dummy_root_comment(instance):
    ...
    if isinstance(instance, Comment):
        raise TypeError('instance object must be one of the next types: (BlogArticle, Page)')
    root_comment = Comment.objects.create(user=user, root=instance, text=fake.text())
    return root_comment
...
def create_dummy_nested_comment(parent):
    ...
    nested_comment = Comment.objects.create(user=user, parent=parent, text=fake.text())
